...
async function verifySignedTransferTx(signedTx) {
    expect(globalThis.Buffer.from(signedTx.signature.data).toString('base64')).toEqual('lpqDMyGG7pdV5IOTJVJYBuGJo9LSu0tHYOlEQ+l+HE8i3u7wBZqOlxMQDtpuGRRNp+ig735TmyBwi6HY0CG9AQ==');
...
describe('roundtrip test', () => {
    ...
    for (const testFile of testFiles) {
        if (/.+\.json$/.test(testFile)) {
            ...
            test(testFile, () => {
                const data = globalThis.Buffer.from(testDefinition.data, 'hex');
